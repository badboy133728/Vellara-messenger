...
returns boolean language sql stable security definer set search_path = public as $$
  select exists (
    ...
    join public.conversation_members cm on cm.conversation_id = m.conversation_id and cm.user_id = uid
    where m.file_path = file_path and m.deleted_at is null
  );
$$;

drop policy if exists "messages_public_read" on storage.objects;
drop policy if exists "messages_member_read" on storage.objects;
create policy "messages_member_read" on storage.objects for select
  using (bucket_id = 'messages' and public.can_read_message_file('messages/' || name, auth.uid()));
...
create policy "messages_auth_upload" on storage.objects for insert
  with check (bucket_id = 'messages' and auth.uid() is not null and auth.uid()::text = (storage.foldername(name))[1]);
